...
public abstract class CharTokenizer extends Tokenizer {
  ...
  public final boolean incrementToken() throws IOException {
    ...
    while (true) {
      ...
      //SPLITTING ON SEPARATORS START
      final int c;
      if (next==-1)
        c = charUtils.codePointAt(ioBuffer.getBuffer(), bufferIndex,ioBuffer.getLength());
      else
        c = next;
      if(bufferIndex+1< ioBuffer.getLength())
        next = charUtils.codePointAt(ioBuffer.getBuffer(),bufferIndex+1,ioBuffer.getLength());
      else
        next = -1;
      //SPLITTING ON SEPARATORS END
      final int charCount = Character.charCount(c);
      bufferIndex += charCount;

      if (isTokenChar(c, prev,next)) {               // if it's a token char
  ...
  //to avoid non whitespace analyzers
