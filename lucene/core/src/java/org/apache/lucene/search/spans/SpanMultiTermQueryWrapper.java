  ...
  public final static SpanRewriteMethod SCORING_SPAN_QUERY_REWRITE = new SpanRewriteMethod() {
    private final ScoringRewrite<SpanOrQuery> delegate = new ScoringRewrite<SpanOrQuery>() {
      ...
      protected void addClause(SpanOrQuery topLevel, Term term, int docCount, float boost, TermContext states) {
        ...
        /*changes done to do span search back to number field
         * after doing range search on numberTrie field 
        */
       final SpanTermQuery q;
       SimpleDateFormat dateFormat;
       if(term.field().equalsIgnoreCase("numberTrie"))
       {
        Term termNew=new Term("number",Integer.toString(NumericUtils.prefixCodedToInt(term.bytes())));
       // Term term1=new Term("number",TrieField.getMainValuePrefix(ft));
        q=new SpanTermQuery(termNew);
       } 
       else if(term.field().equalsIgnoreCase("dateTrie"))
       {
         long val=NumericUtils.prefixCodedToLong(term.bytes()); // converting long to date & spliting in to check for the format
         Date date=new Date(val); 
        
         SimpleDateFormat dateFormatCheck= new SimpleDateFormat();
         dateFormatCheck.setTimeZone(TimeZone.getTimeZone("GMT"));
         String d= dateFormatCheck.format(date);
         ...
         Term termNew=new Term("date",dateFormat.format(date));
       //  Term termNew=new Term("date",term.bytes());
         q=new SpanTermQuery(termNew);
        } 
       else if(term.field().equalsIgnoreCase("moneyTrie"))
       {
         
        
         //System.out.println(NumericUtils.doubleToSortableLong(NumericUtils.sortableIntToFloat(NumericUtils.prefixCodedToInt(term.bytes()))));
         //System.out.println("SHIFT: "+NumericUtils.getPrefixCodedIntShift(term.bytes())+ " To INT "+NumericUtils.prefixCodedToInt(term.bytes())+" TO FLOAT "+NumericUtils.prefixCodedToLong(term.bytes()));
        // System.out.println( Float.toString(NumericUtils.sortableIntToFloat(NumericUtils.prefixCodedToInt(new BytesRef(term.bytes().utf8ToString())))));
        // System.out.println(Float.toString(NumericUtils.sortableIntToFloat(NumericUtils.prefixCodedToInt(term.bytes()))));
        
         Term termNew=new Term("money",Float.toString(NumericUtils.sortableIntToFloat(NumericUtils.prefixCodedToInt(term.bytes()))));
         
       
         //Term termNew=new Term("money",term.bytes());
         q=new SpanTermQuery(termNew);
        } 
       else if(term.field().equalsIgnoreCase("percentageTrie"))
       {
         Term termNew=new Term("percentage",term.bytes());
         q=new SpanTermQuery(termNew);
        } 
