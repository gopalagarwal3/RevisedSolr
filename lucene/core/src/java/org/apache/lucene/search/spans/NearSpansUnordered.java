...
public class NearSpansUnordered extends Spans {
  ...
  public NearSpansUnordered(SpanNearQuery query, AtomicReaderContext context, Bits acceptDocs, Map<Term,TermContext> termContexts)
    throws IOException {
    this.query = query;
    this.nearPre = query.getNearPre();
    this.nearPost=query.getNearPost();
  ...
  private boolean atMatch() {
     int indexSlop = (max.end() - min().start() - totalLength);
     if(ordered.size()==2 && ordered.get(0)== max && ordered.get(1) == min()) //This means the terms are present in the reverse order of the query
       indexSlop *= -1; // for cases with negative left slop such as ~-2,4
     return ((min().doc() == max.doc()) && ( indexSlop <= nearPost)
        && (indexSlop >= nearPre));
