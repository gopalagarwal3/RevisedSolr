...
public class SpanNearQuery extends SpanQuery implements Cloneable {
  ...
  protected int nearPre;
  protected int nearPost;
   ...
   * @param nearPre The left slop value
   * @param nearPost The right slop value
   * @param inOrder true if order is important
   * */
  public SpanNearQuery(SpanQuery[] clauses, boolean inOrder, int nearPre,int nearPost) {
    this(clauses,inOrder,nearPre,nearPost, true);     
  }
  
  public SpanNearQuery(SpanQuery[] clauses, boolean inOrder,int nearPre,int  nearPost,boolean collectPayloads) {
    ...
    for (int i = 0; i < clauses.length; i++) {
      ...
      if (i == 0) {                               // check field
        ...
        throw new IllegalArgumentException("Clauses must have same field.");
      }*/ //removing to enable multi field proximity
      this.clauses.add(clause);
    }
    this.collectPayloads = collectPayloads;
  ...
  public int getNearPre() { return nearPre; }
  
  public int getNearPost() { return nearPost; }
  ...
  public String toString(String field) {
    ...
    buffer.append("], ");
    buffer.append(nearPre);
    buffer.append(".");
    buffer.append(nearPost);
    buffer.append(", ");
  ...
  public SpanNearQuery clone() {
    ...
    SpanNearQuery spanNearQuery = new SpanNearQuery(newClauses,inOrder,nearPre,nearPost);
  ...
  public boolean equals(Object o) {
    ...
    if (nearPre != spanNearQuery.nearPre) return false;
    if (nearPost != spanNearQuery.nearPost) return false;
