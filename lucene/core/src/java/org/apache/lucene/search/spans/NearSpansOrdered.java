...
public class NearSpansOrdered extends Spans {
  private final int allowedPostSlop;
  private final int allowedPreSlop;
  ...
  throws IOException {
    ...
    allowedPreSlop = spanNearQuery.getNearPre();
    allowedPostSlop= spanNearQuery.getNearPost();
    ...
    for (int i = 0; i < clauses.length; i++) {
      ...
      if(clauses[i] instanceof SpanOrQuery)
        orFlag=true;
 ...
 /* public void booleanOrParsing(SpanQuery [] clauses)
  {
    List<SpanNearQuery> listQueries=new ArrayList();
  
    for(int i=0 ;i<clauses.length;i++)
    {
      SpanQuery [] orClauses;
      SpanQuery [] finalClause=null;
      if(clauses[i] instanceof SpanOrQuery)
      {
        SpanOrQuery query=(SpanOrQuery)clauses[i];
        orClauses=query.getClauses();
        finalClause[i]=orClauses[i];
      listQueries.add( new SpanNearQuery(finalClause,true, allowedPreSlop, allowedPostSlop));
  ...
  private boolean shrinkToAfterShortestMatch() throws IOException {
    ...
    for (int i = subSpans.length - 2; i >= 0; i--) {
      ...
      while (true) { // Advance prevSpans until after (lastStart, lastEnd)
        ...
        /*Added the macthSLop conditon inside the loop
         * so it matches for all the spans & advanced accordingly
         */
        assert prevStart <= matchStart;
        if (matchStart > prevEnd) { // Only non overlapping spans add to slop.
          matchSlop = (matchStart - prevEnd);
        }
         match = matchSlop >= allowedPreSlop && matchSlop<=allowedPostSlop;
        if(match==true){
          more=false;
          break;
        }
        else if(allowedPostSlop<matchSlop)
        {
          if (! prevSpans.next()) {
            inSameDoc = false;
            more = false;
          ...
          if(subSpans[i+1].next())
          { matchStart=subSpans[i+1].start();
            matchEnd=subSpans[i+1].end();
          }
          else{
            inSameDoc = false;
          more = false;
   ...
   // boolean match = matchSlop >= allowedPreSlop && matchSlop<=allowedPostSlop;
