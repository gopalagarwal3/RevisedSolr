...
public abstract class QueryParserBase extends QueryBuilder implements CommonQueryParserConfiguration {
  ...
  if (query instanceof PhraseQuery) {
    ((PhraseQuery) query).setNearPre(nearPre);
    ((PhraseQuery) query).setNearPost(nearPost);
  }
  if (query instanceof MultiPhraseQuery) {
    ((MultiPhraseQuery) query).setNearPre(nearPre);
    ((MultiPhraseQuery) query).setNearPost(nearPost);
  }
  ...
  */
  ...
  Query handleQuotedTerm(String qfield, Token term, Token fuzzySlop) throws ParseException {
    ...
    return getFieldQuery(qfield, discardEscapeChar(term.image.substring(1, term.image.length()-1)),0, s);
  }
