...
public class SpanQueryLexer {
  ...
  private final static String BUTNOT_identifier = "!";
  private final static String RangeProximity_identifier = "@";
  ...
  private final static String PROXIMITY_START = "<";
  private final static String PROXIMITY_END = ">";
  ...
  private final static String NOT_NEAR_MODIFIERS = BUTNOT_identifier+"~(?:(\\d+)(?:,(\\d+))?)?";
  private final static String NEAR_PRE_POST = RangeProximity_identifier+"~(?:(-?\\d+)(?:,-?(\\d+))?)?";
  private final static String NEAR_CLOSING_MODIFIERS = "("+NEAR_MODIFIERS+"|"+NOT_NEAR_MODIFIERS+"|"+NEAR_PRE_POST+")?";
  ...
  //Multifield proximity : <fileDataEnglish:TAX number:[0 100]>~-5,5 
  //<\w+:[A-Z]+\s+\w+:(?:[#]?[A-Z]+|\[\d+ \d+\])>\~[-]?\d+,\d+
  private final static String RANGE_IDENTIFIER = "(?:\\[(\\*?|\\d+) (\\*?|\\d+)\\])";
  ...
  private SQPClause buildNearOrNotNear(Matcher m, List<SQPToken> tokens, SQPOpenClause open) 
      throws ParseException{
    int nearPre = SQPNearClause.NEAR_DEFAULT;
    int nearPost = SQPNearClause.NEAR_DEFAULT;
    //try for not near first, return early
    if (m.group(G.NEAR_PARAM.ordinal()) != null && m.group(G.NEAR_PARAM.ordinal()).startsWith(BUTNOT_identifier)){
    ...
    //adding the below condition to implement Range proximity
    if(m.group(G.NEAR_PARAM.ordinal()) != null && m.group(G.NEAR_PARAM.ordinal()).startsWith(RangeProximity_identifier))
    {
      
      if (m.group(G.NEAR_PRE.ordinal()) != null){
        nearPre = Integer.parseInt(m.group(G.NEAR_PRE.ordinal()));
        nearPost = nearPre;
      }
      if (m.group(G.NEAR_POST.ordinal()) != null){
        nearPost = Integer.parseInt(m.group(G.NEAR_POST.ordinal()));
      }
      //contents of this clause start at 1 after tokenOffsetStart
      SQPNearClause clause;
      if (nearPre<0)
      {clause = new SQPNearClause(open.getTokenOffsetStart()+1, tokens.size(),
          open.getStartCharOffset(), m.start(),
          open.getType(), false, false, nearPre,nearPost);}
      else
      {clause = new SQPNearClause(open.getTokenOffsetStart()+1, tokens.size(),
          open.getStartCharOffset(), m.start(),
          open.getType(), false, true, nearPre,nearPost);}
      tryToAddBoost((SQPBoostableToken)clause, m);
      return clause; 
    ...
    if (m.group(G.NEAR_SLOP.ordinal()) != null){
      ...
      nearPost =  Integer.parseInt(m.group(G.NEAR_SLOP.ordinal()));
    }
    ...
    SQPNearClause clause = new SQPNearClause(open.getTokenOffsetStart()+1, tokens.size(),
        open.getStartCharOffset(), m.start(),
        open.getType(), hasParams, inOrder,-nearPost ,nearPost);
    tryToAddBoost((SQPBoostableToken)clause, m);
    return clause;
  }
  ...
  private void addField(String term, int nearDepth, List<SQPToken> tokens) throws ParseException{
    ...
    }*/ // We want to run proximity queries in SpanNearQueries. Hence, commenting this
