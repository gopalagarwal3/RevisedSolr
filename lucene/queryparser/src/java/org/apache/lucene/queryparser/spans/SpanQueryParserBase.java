...
public abstract class SpanQueryParserBase extends AnalyzingQueryParserBase{
  ...
  public static final int UNSPECIFIED_SLOP = Integer.MIN_VALUE;
   ...
   */
  protected SpanNearQuery newNearQuery(SpanQuery[] queries, boolean inOrder,int nearPre, int nearPost, boolean collectPayloads){
    return new SpanNearQuery(queries,inOrder,nearPre,nearPost,collectPayloads);
  }
 ...
 */
  ...
  MultiTermQuery query = null;
  SimpleDateFormat df;
    /* These changes are done to make the Trie 
     * Range queries work with span
     */
 try{
    if(field.equalsIgnoreCase("numberTrie"))
    {
       query=NumericRangeQuery.newIntRange(field, Integer.parseInt("4"),Integer.parseInt(start), Integer.parseInt(end), startInclusive, endInclusive);
      query.setRewriteMethod(getMultiTermRewriteMethod(field));
     // return new SpanMultiTermQueryWrapper(query);
      
    }
    else if(field.equalsIgnoreCase("dateTrie"))
        {   
     /*final NumericRangeQuery query=NumericRangeQuery.newLongRange(field, Integer.parseInt("4"),dateField.parseMath(null, start.concat("T00:00:00Z")).getTime(), 
               dateField.parseMath(null,end.concat("T00:00:00Z")).getTime(), startInclusive, endInclusive);*/
      ...
      query=NumericRangeQuery.newLongRange(field, Integer.parseInt("4"), df.parse(start.concat("T00:00:00Z")).getTime(),df.parse(end.concat("T00:00:00Z")).getTime(), startInclusive, endInclusive);
      query.setRewriteMethod(getMultiTermRewriteMethod(field));
     // return new SpanMultiTermQueryWrapper(query);
      
        }
    else if(field.equalsIgnoreCase("moneyTrie")
        ||field.equalsIgnoreCase("percentageTrie")){
     query=NumericRangeQuery.newFloatRange(field, Integer.parseInt("4"),Float.parseFloat(start), Float.parseFloat(end), startInclusive, endInclusive);
      ...
      /*These changes are done to convert the ranges in span to sortable values
       * Cases are different for date & others
       */
      if( field.equalsIgnoreCase("number")||field.equalsIgnoreCase("money")||field.equalsIgnoreCase("percentage"))
      {
        double startDouble = Double.parseDouble(start);
        double endDouble = Double.parseDouble(end);
       start = NumberUtils.double2sortableStr(startDouble);
       end = NumberUtils.double2sortableStr(endDouble);
      }
      else if( field.equalsIgnoreCase("date"))
      {
        int countStart=start.split("-").length;
        int countEnd=end.split("-").length;
        ...
        if(countStart==3)
          ...
          df=new SimpleDateFormat("yyyy");
        start=NumberUtils.long2sortableStr(df.parse(start).getTime());
        ...
        end=NumberUtils.long2sortableStr(df.parse(end).getTime());
      }
    ...
    {
      System.out.println(" Got the exception in creating range query in spnaQueryParserBase");
    }
   return new SpanMultiTermQueryWrapper(query);
  ...
  @Override
  protected Query getFieldQuery(String field, String queryText, int nearPre,int nearPost)
      throws ParseException {
    Query query = getFieldQuery(field, queryText, true);

    if (query instanceof SpanNearQuery) {
    //  if (((SpanNearQuery)query).getNearPre() != nearPre){
//        slop = (spanNearMaxDistance > -1 && slop > spanNearMaxDistance) ? spanNearMaxDistance : slop;
        ...
        query = new SpanNearQuery(clauses, true,nearPre,nearPost);
      //}
    }
  ...
  protected Query newFieldQuery(Analyzer analyzer, String fieldName, String termText, boolean quoted)
    ...
    if (numTokens == 0){
      ...
      /*These changes are done for the exact value match
       * in our span queries . To convert them to sortable string
       */
      SimpleDateFormat df;
      if(fieldName.equalsIgnoreCase("number")||fieldName.equalsIgnoreCase("money")||fieldName.equalsIgnoreCase("percentage")){
      String sort_value=NumberUtils.double2sortableStr(bytes.utf8ToString());
      bytes=  new BytesRef(sort_value.getBytes());
      }
      else if (fieldName.equalsIgnoreCase("date"))
      {
       String value=bytes.utf8ToString();
        int countSplit=value.split("-").length;
        ...
        if(countSplit==3)
          ...
          df=new SimpleDateFormat("yyyy");
        try {
          String sort_value=NumberUtils.long2sortableStr(df.parse(value).getTime());
          bytes=  new BytesRef(sort_value.getBytes());
        } catch (java.text.ParseException e) {
          throw new RuntimeException(" Exception while parsing the date value in query in class AbstractSpanQueryParser for the value:"+value);
        }
      ...
      if (quoted || getAutoGeneratePhraseQueries() == true) {
        return new SpanNearQuery(ret,true,0,0); 
      } else {
  ...
  *         only empty queries
  ...
  protected SpanQuery buildSpanNearQuery(List<SpanQuery> clauses, Boolean inOrder,int nearPre, int nearPost) throws ParseException {
    if (clauses == null || clauses.size() == 0)
      return getEmptySpanQuery();
    ...
    if (nonEmpties.size() == 1){
      ...
      if (child instanceof SpanNearQuery){
        ...
        child = new SpanNearQuery(childsClauses, inOrder, nearPre, nearPost);
      }
    }

    if (nearPost == UNSPECIFIED_SLOP){
      nearPost = getPhraseSlop();
    } else if  (spanNearMaxDistance > -1 && nearPost > spanNearMaxDistance) {
      nearPost = spanNearMaxDistance;
    }
    ...
    SpanQuery[] arr = nonEmpties.toArray(new SpanQuery[nonEmpties.size()]);
    return new SpanNearQuery(arr,localInOrder,nearPre,nearPost);
  }
  ...
  */
  protected Query specialHandlingForSpanNearWithOneComponent(String field,
      String termText, 
       Boolean ancestralInOrder, int nearPre, int nearPost) throws ParseException {
    ...
    if (q instanceof SpanNearQuery){
      ...
      return buildSpanNearQuery(Arrays.asList(childsClauses),ancestralInOrder, nearPre,nearPost);
    }
    return q;
  }
