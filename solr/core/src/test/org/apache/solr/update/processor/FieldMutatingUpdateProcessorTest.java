...
public class FieldMutatingUpdateProcessorTest extends UpdateProcessorTestBase {
  ...
  //Test case to test the parse-int update processor/ Patch: SOLR-4864  (added by amalto)
  public void testRegexReplacePatch() throws Exception {

    SolrInputDocument d = null;

    // regardless of chain, all of these should be equivilent
    for (String chain : Arrays.asList("parse-int" )) {

      // simple clone
      d = processAdd(chain,       
                     doc(f("id", "1111"),
                         f("number", "N/12/45/5/68")));
      assertNotNull(chain, d);
       
       assertEquals("N/12/45/5/68", d.getFieldValues("number"));
       assertEquals("68", d.getFieldValues("numberTrie"));}
    }
