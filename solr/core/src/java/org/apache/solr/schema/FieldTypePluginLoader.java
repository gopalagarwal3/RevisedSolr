...
public final class FieldTypePluginLoader 
  ...
  protected FieldType create( SolrResourceLoader loader, 
    ...
    if (null != analyzer) {
      ...
      if (ft instanceof EntityTextField) {
        if (null == multiAnalyzer) {
          multiAnalyzer = constructMultiTermAnalyzer(queryAnalyzer);
          ((EntityTextField)ft).setIsExplicitMultiTermAnalyzer(false);
        } else {
          ((EntityTextField)ft).setIsExplicitMultiTermAnalyzer(true);
        }
        ((EntityTextField)ft).setMultiTermAnalyzer(multiAnalyzer);
      }
    }
